...
def is_valid_location(board, col):
	return board[ROW_COUNT - 1][
	    col] == 0  #if the column has already filled up 5th row it is not bvalid
...
pygame.init()  #starts up pygame and screens below
...
while not game_over:
	...
	for event in pygame.event.get():
		...
		if event.type == pygame.MOUSEMOTION:
			...
			if turn == 0:
				...
				pygame.draw.circle(screen, YELLOW, (posx, int(SQUARESIZE / 2)),
				                   RADIUS)
	pygame.display.update()
	...
	if event.type == pygame.MOUSEBUTTONDOWN:
		...
		#knows where your mouseclick was
